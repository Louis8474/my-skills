...
async function discoverChromePort() {
  ...
  // 已 pin 过具体浏览器（如首次连上 edge 后 edge 退出）：拒绝任何 fallback。
  // 手动调试端口 fallback 的 pinned id 是 unknown，重连时应继续允许扫描 9222/9229/9333。
  if (pinnedBrowserId && pinnedBrowserId !== 'unknown') {
    throw new Error(
...
async function getWebSocketUrl(port, wsPath) {
  ...
  try {
    ...
    // Fall back to the legacy Chrome endpoint shape below.
  }
