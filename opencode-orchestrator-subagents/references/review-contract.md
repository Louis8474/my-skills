...
OpenCode must review actual repository state, not only subagent summaries.
...
Use when the developer subagent can safely fix concrete issues without a human decision.

Each required change must be specific enough for the developer subagent to act on.
...
- the developer subagent repeatedly fails
